...
GRAPH_BOTTOM = 0.21  # posición del borde inferior del gráfico (0-1)
GRAPH_LEFT = 0.1  # posición del borde izquierdo del gráfico (0-1)
GRAPH_RIGHT = 0.95  # posición del borde derecho del gráfico (0-1)
EARTH_RADIUS_KM = 6371  # radio del cuerpo celeste en km (1737=Luna, 6371=Tierra)
# ================================
...
def load_csv(path):
    """Load CSV: soporta Artemis II (sep=';', decimal=',') y lunar_orbit (sep=',', decimal='.')"""
    # Intentar detectar el formato leyendo la primera línea
    with open(path, 'r') as f:
        first_line = f.readline()
    
    # Si contiene punto y coma, es Artemis; si contiene coma normal, es lunar_orbit
    if ';' in first_line:
        # Formato Artemis II
        ...
        # Formato lunar_orbit (con header y separador coma)
        df = pd.read_csv(path, sep=',', low_memory=False)
        # Renombrar columnas si es necesario (lunar_orbit tiene nombres con espacios)
        # Buscar columnas que contengan X, Y, Z, VX, VY, VZ
        col_map = {}
        for col in df.columns:
            col_upper = col.strip().upper()
            if 'CALENDAR' in col_upper and 'DATE' in col_upper:
                col_map[col] = 'time'
            elif 'JDTDB' in col_upper or 'JULIAN' in col_upper:
                pass  # No mapear JDTDB, preferimos Calendar Date
            elif col_upper.startswith('X') and 'CALENDAR' not in col_upper:
                col_map[col] = 'x'
            elif col_upper.startswith('Y'):
                col_map[col] = 'y'
            elif col_upper.startswith('Z'):
                col_map[col] = 'z'
            elif col_upper.startswith('VX'):
                col_map[col] = 'vx'
            elif col_upper.startswith('VY'):
                col_map[col] = 'vy'
            elif col_upper.startswith('VZ'):
                col_map[col] = 'vz'
...
def main():
    ...
    circle = Circle((0, 0), EARTH_RADIUS_KM, fill=False, edgecolor='blue', linewidth=1, linestyle='-')
    ax.add_patch(circle)
    ...
    # Slider (debajo de los botones)
    ax_slider = plt.axes([0.10, 0.02, 0.85, 0.04])
    ...
    ax_play = plt.axes([0.12, 0.07, 0.08, 0.04])
    btn_play = Button(ax_play, 'Play')
    
    ax_prev = plt.axes([0.22, 0.07, 0.08, 0.04])
    btn_prev = Button(ax_prev, 'Prev')
    
    ax_next = plt.axes([0.32, 0.07, 0.08, 0.04])
